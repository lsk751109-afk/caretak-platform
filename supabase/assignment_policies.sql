...
-- Security-definer helpers prevent circular RLS checks across related tables.
create or replace function public.is_own_caregiver(target_caregiver_id uuid) returns boolean language sql stable security definer set search_path = public as $$
  select exists (select 1 from public.caregivers c where c.id = target_caregiver_id and c.user_id = auth.uid());
$$;
create or replace function public.guardian_can_view_caregiver(target_caregiver_id uuid) returns boolean language sql stable security definer set search_path = public as $$
  select exists (select 1 from public.matching m join public.care_requests cr on cr.id = m.request_id join public.guardians g on g.id = cr.guardian_id where m.caregiver_id = target_caregiver_id and g.user_id = auth.uid());
$$;
create or replace function public.caregiver_can_view_request(target_request_id uuid) returns boolean language sql stable security definer set search_path = public as $$
  select exists (select 1 from public.matching m join public.caregivers c on c.id = m.caregiver_id where m.request_id = target_request_id and c.user_id = auth.uid());
$$;
